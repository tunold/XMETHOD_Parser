...
debug_mode=st.checkbox("🔍 Enable debug mode (show raw XML / parser trace)",value=False)
...
if uploaded:
    try:
        ...
        if parsed.get("_errors"):
            for err in parsed["_errors"]:
                st.warning(f"⚠️ {err}")
        if not parsed.get("Layers"):
            st.warning("⚠️ No valid layer structure found — possibly incomplete or a template.")
        if not parsed.get("Elements"):
            st.warning("⚠️ No element data found — this file may not contain fit results.")

        # Debug view
        if debug_mode:
            st.subheader("🧩 Debug: Parsed JSON Structure")
        ...
        with st.expander("📊 Layer Composition (Stacked Bar)",expanded=True):
            ...
            if df.empty:
                ...
                fig=px.bar(df,x="Layer",y=yfield,color="Element",barmode="stack",
        ...
        c1,c2=st.columns(2)
        with c1:
            st.download_button("💾 Save Full JSON (raw structure)",
                data=full_json,file_name=f"{sample}_full.json",mime="application/json")
        with c2:
            st.download_button("📘 Save Summary JSON",
                data=summary_json,file_name=f"{sample}_summary.json",mime="application/json")
